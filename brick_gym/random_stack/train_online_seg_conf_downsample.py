...
class LowResModel(torch.nn.Module):
    def __init__(self, resnet):
        ...
        self.backbone = bg_resnet.ResnetBackbone(resnet)
        with torch.no_grad():
            x = torch.zeros(1,3,256,256).cuda()
            x = self.backbone(x)
            output_dim = x.shape[1]
        self.conv = torch.nn.Conv2d(output_dim, 7 + 2, kernel_size=1)
    
    def forward(self, x):
        x = self.backbone(x)
        return self.conv(x)
model = LowResModel(resnet).cuda()
...
def rollout(
    ...
    with torch.no_grad():
        ...
        for ep_start in tqdm.tqdm(range(0, num_episodes, args.num_processes)):
            ...
            batch_masks = [
                    numpy.array(
                        Image.fromarray(mask).resize((8,8), Image.NEAREST))
            ...
            for step in range(steps_per_episode):
                ...
                y, x = numpy.unravel_index(locations, (8, 8))
                #xy = list(zip(x, y))
                '''
                ...
                batch_masks = [
                        numpy.array(
                            Image.fromarray(mask).resize((16,16),
                            Image.NEAREST))
                        for mask in batch_masks]
                '''
                instance_ids = [

...
""" From Chris Xie:
    https://github.com/chrisdxie/uois/blob/uois2d/src/networks.py
...
class Conv2d_GN_ReLU(nn.Module):
    ...
    def __init__(self,
            in_channels, out_channels, num_groups, ksize=3, stride=1):
...
class Upsample_Concat_Conv2d_GN_ReLU_Multi_Branch(nn.Module):
    """ Implements a module that performs
        ...
        The Upsample operation consists of a Conv2d_GN_ReLU that
        reduces the channels by 2, followed by bilinear sampling
        
        Note: in_channels is number of channels of ONE of the inputs
    ...
    def __init__(self,
        ...
        self.conv_gn_relu = Conv2d_GN_ReLU(
                int(in_channels//2 * (num_encoders+1)),
                out_channels,
                num_groups)

    def forward(self, x, skips):
        """ Forward module
            @param skips: a list of intermediate skip-layer torch tensors from
            each encoder
        """
...
class UNet_Decoder(nn.Module):
    ...
    def __init__(self, num_encoders, feature_dim, output_dim, coordconv=False):
    ...
    def build_network(self):
        ...
        if self.coordconv:
            ...
            self.layer6 = Conv2d_GN_ReLUx2(
                    self.fd+2, self.fd, self.fd, ksize=1)
            self.layer7 = Conv2d_GN_ReLUx2(
                    self.fd, self.fd, self.fd, ksize=1)        

        # This puts features everywhere, not just nonnegative orthant
        self.last_conv = nn.Conv2d(self.fd, self.od, kernel_size=3,
    ...
    def forward(self, encoder_list):
        ...
        out = torch.cat([x[0] for x in encoder_list], dim=1) # channel dimension
        ...
        if self.coordconv:
            b, c, h, w = out.shape
            y = 2. * torch.arange(h) / (h-1) - 1.
            x = 2. * torch.arange(w) / (w-1) - 1.
            y = y.view(1,1,h,1).expand(b,1,h,w).to(out.device)
            x = x.view(1,1,1,w).expand(b,1,h,w).to(out.device)
            out = torch.cat([out, y, x], dim=1)

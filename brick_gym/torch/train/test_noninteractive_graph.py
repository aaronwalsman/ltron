...
def test_graph(
    ...
    target_graphs = {}
    visible_target_graphs = {}
    predicted_graphs = {}
    
    max_instances_per_step = 1000
        ...
        with torch.no_grad():
            ...
            for i, brick_list in enumerate(step_brick_lists):
                ...
                if not use_ground_truth_segmentation:
                    y = brick_list.pos[:,0]
                    x = brick_list.pos[:,1]
                    segment_id = step_tensors[
                            'segmentation_render'][[i]*len(y),y,x]
            ...
            for i in range(test_env.num_envs):
                ...
                graph_states.append(BrickGraph(
                ...
                if valid_scene:
                    ...
                    visible_instances = torch.unique(
                            step_tensors['segmentation_render'][i])
                    visible_instance = set(
                            int(inst) for inst in visible_instances)
                    visible_graph = target_graph.clone()
                    for j in range(visible_graph.instance_label.shape[0]):
                        if j not in visible_instances:
                            visible_graph.instance_label[j,0] = 0
                    visible_target_graphs[i, scene_index[i]] = visible_graph
                '''
                #-----------------------------------------------------------
            ...
            if dump_debug:
                ...
                if 'cluster_center' in head_features:
                    ...
                    center_voting_offsets = [
                            None for _ in range(test_env.num_envs)]
    ...
    for key, predicted_graph in predicted_graphs.items():
        ...
        for instance in unmatched_instances:
            unmatched_class_label = int(target_graph.instance_label[instance])
            ...
            extant_classes.add(unmatched_class_label)
    extant_classes.remove(0)
    
    mAP, class_ap = evaluation.instance_map(
            instance_class_predictions, class_false_negatives, extant_classes)
    
    print('-'*80)
    print('Full Graph Instance mAP: %.01f'%(mAP*100))
    for class_label, class_ap in class_ap.items():
        print('  %i: %f'%(class_label, class_ap))
    
    #-----------------------------------
    assert visible_target_graphs.keys() == predicted_graphs.keys()
    instance_class_predictions = []
    class_false_negatives = {}
    extant_classes = set()
    for key, predicted_graph in predicted_graphs.items():
        p_instance_label = torch.softmax(predicted_graph.instance_label, dim=1)
        target_graph = visible_target_graphs[key]
        for i, segment_id in enumerate(predicted_graph.segment_id):
            correct_label = int(target_graph.instance_label[segment_id,0])
            extant_classes.add(correct_label)
            for j in range(p_instance_label.shape[1]):
    ...
    print('-'*80)
    print('Visible Instance mAP: %.01f'%(mAP*100))
    for class_label, class_ap in class_ap.items():
        print('  %i: %f'%(class_label, class_ap))
    #-----------------------------------
    ...
    print('-'*80)
    print('Full Edge AP: %.01f'%(edge_ap*100))
    ...
    _, _, edge_ap = evaluation.edge_ap(predicted_edges, target_edges)
    print('-'*80)
    print('Visible Edge AP: %.01f'%(edge_ap*100))

...
class HandspaceBrickInserter(LtronGymComponent):
    def __init__(
        ...
        self.brick_type_to_id = shape_ids
        self.id_to_brick_type = {value:key for key, value in shape_ids.items()}
        ...
        self.action_space = Dict({
            'shape' : Discrete(max(self.id_to_brick_type.keys())+1),
    ...
    def step(self, action):
        ...
        if num_instances > self.max_instances:
            success = False
        elif (action['shape'] in self.id_to_brick_type and
            action['color'] in self.id_to_color_name
        ):
            scene = self.hand_component.brick_scene
            scene.clear_instances()
            brick_type = self.id_to_brick_type[action['shape']]

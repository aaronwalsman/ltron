...
class PixelCursor(CursorComponent):
    def __init__(self,
        max_instances_per_scene,
        ...
        image_height = pos_render_component.height
        image_width = pos_render_component.width
        assert neg_render_component.height == image_height
        assert neg_render_component.width == image_width
        
        action_space = {
            'activate' : Discrete(2),
            'select' : PixelSpace(height, width),
        }
        self.action_space = Dict(action_space)
        observation_space = {
            'position' : PixelSpace(height, width),
        }
        if observe_selected:
            observation_space['selected'] = Discrete(2)
        self.observation_space = Dict(observation_space)
